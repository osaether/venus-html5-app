...
const hasTwoColumns = metricsContainer => {
  if (metricsContainer.childElementCount === 1) return false
  const first = metricsContainer.children[0]
...
export default class Metrics extends Component {
  ...
  componentDidMount() {
    if (this.state.layoutCols === 1 && this.metricsRef.current && hasTwoColumns(this.metricsRef.current)) {
